...
| 出错时机 | 返回 | 为什么 |
|---|---|---|
| 还没吐出任何字节（准备期、上游建流失败） | 标准 JSON + 正确状态码（404 / 401 / 429 / 502…） | 响应头还没提交，能返回真实状态 |
| 已经吐出内容之后 | HTTP 200 + `event: error` | 头早提交了，改不回来；而且这本来就是「部分成功」 |

所以 `meta` 事件发在**上游握手成功之后**——成功路径的事件顺序完全不变（`meta` 仍是第一个），但建流阶段失败时不会先写个 200 再用 SSE 找补。
...
脚本会自己编译、拉起假上游（`:19090`）和网关（`:18080`）、跑完全部断言、清理进程。**190 项断言，全部离线，无需真实 Key**：

```
共 190 项断言：通过 190，失败 0
...
├── scripts/verify.sh       全功能验收脚本（190 项断言）
└── Makefile
```
