...
>
> 生产上要做分布式限流有四条路，按「同步代价」排序：
>
> | 方案 | 代价 | 适用 |
> |---|---|---|
> | 静态切分（每副本配 `rate/N`） | 零协调，但副本数变化要重配、负载不均时浪费配额 | 副本数固定 |
> | Redis 集中式令牌桶（Lua 脚本保证原子性） | 每请求多一次 Redis 往返，Redis 是单点 | 配额贵、不能超 |
> | 本地桶 + 周期同步 | 有滞后，同步周期内可能超发 | 高 QPS |
> | AIMD（撞 429 降速、平稳时加速） | 必然超发一部分，靠上游拒绝兜底 | 上游不告知配额 |
>
