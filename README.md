...
脚本会自己编译、拉起假上游（`:19090`）和网关（`:18080`）、跑完全部断言、清理进程。**176 项断言，全部离线，无需真实 Key**：

```
共 176 项断言：通过 176，失败 0
...
单元测试有 46 个用例，覆盖纯逻辑部分：协议翻译（两个适配器的请求/响应双向映射、消息序列归一、结构化输出的两种表达）、SSE 解析、指数退避区间、令牌桶与按模型隔离、错误码分流、JSON 抽取与 Schema 校验、模板版本递增与落盘、指标聚合与分位数。
...
├── scripts/verify.sh       全功能验收脚本（176 项断言）
└── Makefile
```
